    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <a href="{{ route('teachers.create') }}" class="bg-blue-500 hover:bg-blue-700 font-bold py-2 px-4 rounded mb-4 inline-block">Añadir Profesor</a>
                <table class="min-w-full">
                    ...
                    <tbody>
                        @forelse($teachers['data'] ?? [] as $teacher)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500">
                                    <a href="{{ route('teachers.show', $teacher['id']) }}" class="text-blue-600 hover:text-blue-900">Ver</a>
                                    <a href="{{ route('teachers.edit', $teacher['id']) }}" class="text-indigo-600 hover:text-indigo-900 ml-4">Editar</a>
                                    <form action="{{ route('teachers.destroy', $teacher['id']) }}" method="POST" class="inline-block ml-4" onsubmit="return confirm('¿Estás seguro de que quieres eliminar este profesor?');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                            <tr><td colspan="4" class="text-center py-4">No se encontraron profesores.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
